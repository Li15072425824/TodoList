...
通过修改项目根目录下的 `.env.development` 文件，可以在本地后端和线上后端之间灵活切换。

### 1. 链接线上后端（直连模式）
由于本地开发环境可能无法通过代理连接线上 Vercel 服务（网络超时），推荐采用**直连模式**。
前端浏览器直接请求线上接口，绕过本地 Vite 代理。

**前提：** 必须确保后端已部署并支持 CORS（跨域）。

配置如下：
```properties
VITE_USE_MOCK=false
VITE_API_BASE_URL=https://todo-list-psi-sooty-85.vercel.app
```

**重要操作：**
修改完代码后，请务必**重新部署后端**到 Vercel，以使 CORS 配置生效。
部署完成后，重启本地开发服务器，前端将直接请求 `https://todo-list-psi-sooty-85.vercel.app/api/get-data`。

### 2. 链接本地 Node 后端
若需调试本地 Node 服务，请修改配置如下：
```properties
VITE_USE_MOCK=false
# 注释掉线上地址，或改为 http://localhost:3000
VITE_API_BASE_URL=http://localhost:3000
```
并确保已运行 `node api/server.js` 启动本地服务。
