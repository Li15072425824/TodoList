...
### 1. 链接线上后端（代理模式 - 推荐）
为了解决浏览器跨域（CORS）限制，推荐使用 Vite 的**代理模式**。
前端请求发送到本地开发服务器（如 `localhost:5173/api/...`），由 Vite 转发给线上后端。
...
**注意：**
- 修改 `.env.development` 后需要**重启本地开发服务器**。
- `src/api/request.js` 中已关闭 `baseURL`，确保请求走相对路径。
